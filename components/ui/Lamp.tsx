...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        // flex-col
        "relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-950 w-full rounded-md z-0",
        className
      )}
    >
      <div className="relative flex  flex-1 scale-y-125 items-center justify-center isolate z-0 max-sm:scale-50">
        ...
        <div className="absolute inset-auto z-30 h-44 w-full -translate-y-[12.5rem] bg-slate-950 "></div>
      </div>

      {/* px-5 */}
      {children && (
        <div className="relative z-40 flex -translate-y-80 flex-col items-center">
          {children}
        </div>
      )}
    </div>
